...
failure forcing the score to zero. We release the harness and seeds and report uniform single-model sweeps
of \emph{Claude Opus~4.7} and \emph{Gemini~3.1~Pro Preview} over all ten
tasks on Apple M1~Pro at matched iteration budgets. In-distribution
self-speedups span $1.00\times$ to $10.7\times$; the two models are
within 5\% on five tasks (notably HMC at $\sim$10.6$\times$, the high
end), each model wins outright on three or four of the remainder, and
correctness-failure rates differ by an order of magnitude (Opus 13/110,
Pro 0/95 of fresh-run candidates). \emph{The held-out evaluation is
sharper than the in-distribution one and exposes overfitting that the
geometric mean hides --- and the two models overfit
asymmetrically.} On HMC, Opus's \texttt{template <uint D>} dispatch
that took $d{=}8$ from 121 to 970\,GFLOPS hard-codes branches for
$d{\in}\{8,16,32\}$ and \emph{returns wrong answers} at the held-out
$d{=}24$, where Pro's runtime-$d$ unroll generalises cleanly at
$17.8\times$ over the seed. On LBM, both models pin a fixed
threadgroup geometry tuned for the training sizes and regress at
$192^2$. Pro is never strictly worse than Opus on held-out
across the ten tasks. Metal-specific syntax recurs as an
out-of-distribution failure across iterations. The benchmark is
designed for fast iteration: each candidate compiles, runs, verifies,
and scores in seconds.
\end{abstract}
...
(\emph{O}) and gemini-3.1-pro-preview (\emph{P}) --- over the ten tasks at
...
P = gemini-3.1-pro-preview. \emph{In-dist.\ $\times$} = best / seed,
...
hmc      & 10.6 & 10.7  & ---    & 9.85\% & \textbf{FAIL}  & 17.8           & \textbf{O wrong at $d{=}24$}; P clean \\
\bottomrule
\end{tabular}
\end{table}
...
1.4\% (O 0.0932 vs P 0.0870). Outside HMC the models split: Opus
wins clearly on saxpy ($1.25$ vs $1.00$), nbody ($2.83$ vs $2.00$),
lbm ($1.46$ vs $1.06$), and wave3d ($1.26$ vs $1.00$); Pro wins on
...
neighbour-list reorganisation) favours Pro. Saturated tasks (saxpy,
heat2d, wave3d) sit above 78\% of effective DRAM ceiling on the seed;
heat2d is the cleanest demonstration that the score is a \emph{hard}
signal --- on Opus zero candidates strictly dominated the seed across
all three sizes and the incumbent stayed at iter~0. Pro had
...
fft3d, gradshaf, lj. Gradshaf and lj match (or beat) their
in-distribution speedups; fft3d $256^3$ extrapolates past the largest
training size $128^3$ for both models.
...
off target. Pro arrived at the same template-D speedup
...
$0.50\times$ Pro) at the held-out $192^2$ grid: each pins a fixed
...
cannot recover the loss. \textbf{Pro is never strictly worse than
...
Opus vs 22/95 for Pro, with Pro exploring more aggressively per
iteration but never breaking correctness, and Opus producing more
correct-but-slow candidates. Pro's wall-time per iteration is
...
Both Opus and Pro independently arrive at this structural change.
...
correctness at the held-out $d{=}24$; Pro kept a runtime-$d$ fallback
...
score hides. The Opus--Pro asymmetry is concrete: at matched iteration
budgets the in-distribution scores are close, but Pro's optimisations
