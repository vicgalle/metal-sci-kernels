...
on HMC, Stockham auto-sort + radix-4 on 3D FFT) yet still mishandle
Metal-specific concurrency and naming syntax, miss resource-pressure
tradeoffs that gate occupancy, and stall at single-digit percent of peak
on irregular-memory tasks (Lennard-Jones with cell lists). The benchmark is designed for fast iteration: each candidate
...
next run cleanly past the trap. On fft3d, two of ten Opus~4.7 iterations
(1 and 6) tripped the same Metal-specific naming collision: a local
\texttt{uint half = N >> 1u;} clashes with MSL's built-in \texttt{half}
type (fp16), producing a cascading parse error that is not a CUDA failure
mode. Iteration~1 also placed the threadgroup-size attribute as a suffix
\emph{after} the parameter list rather than \emph{before} the kernel
declaration --- a different syntactic mistake from LBM's, but the same
underlying gap: Metal's attribute grammar is structurally distinct from
CUDA \texttt{\_\_launch\_bounds\_\_}.
\emph{(iii) Irregular and shuffle-bound tasks remain hard.} Lennard-Jones
with cell lists sits at $\sim$1\% of FP32 peak after 10 iterations across
both Claude and Gemini. The atomic scatter in \texttt{build\_cells} and the
variable-occupancy neighbour-cell loop are not patterns the models
recombine effectively in our budget. fft3d shows a related but distinct
failure: by iteration~10 Opus~4.7 has recovered the textbook optimisation
--- Stockham auto-sort, mixed radix-4 + radix-2 final stage, cooperative
twiddle-table precompute --- yet the candidate scores only $1.03\times$
over the seed because it carries two ${\sim}8$\,KB threadgroup ping-pong
buffers plus the twiddle table (${\sim}20$\,KB total), pushing TG-memory
occupancy above the level at which the M1~Pro can resident-schedule
enough threadgroups to saturate DRAM at $128^3$. The algorithmic recall
is correct; the resource-pressure reasoning is missing. Together with LJ,
fft3d is the second task in the suite where 10 iterations of a frontier
model fails to clear a $2\times$ gain.
