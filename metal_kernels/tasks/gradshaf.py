...
class GradShafranovTask(Task):
    spec = TaskSpec(
        ...
        kernel_signatures=(
            ...
            "dispatch.\n"
            "\n"
            "Step dispatch: 2-D, threadsPerGrid = (NR, NZ), one thread "
            "per output cell — guard with `if (i >= NR || j >= NZ) "
            "return;`. Boundary cells MUST copy psi_in -> psi_out "
            "unchanged. Each thread MUST update exactly one cell; the "
            "host will not shrink the dispatch."
        ),
